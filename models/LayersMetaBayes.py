...
class Gaussian_MetaBayes(object):
    """Object that : 
        - This object does not use a variable change, as proposed originally in BBB because the update rule ensure sigma>0 (as long as the sigma*grad<1)
...
class Linear_MetaBayes(Module):
    ...
    def __init__(self, in_features: int, out_features: int, 
        ...
        # Define weight parameters, sigma must be define before mu to be compatible with Bayesian_Optimizers.py
        ...
        self.sigma_init = 0.5*math.sqrt(1/out_features)
    ...
    def reset_parameters(self) -> None:
        ...
        # init.uniform_(self.weight_sigma,1e-4, self.sigma_init)
        # sigma_p=self.bound/3
        # self.weight_sigma.data = (sigma_p**2  + self.weight_mu.data**2 )**0.5
        ...
        if self.bias is not None:
            ...
            # init.uniform_(self.bias_sigma,1e-4, self.sigma_init)
    ...
    def forward(self, x: Tensor, samples: int) -> Tensor:
        ...
        if self.bias:
            ...
            y = torch.einsum('soi, sbi -> sbo', W, x) + B[:, None]
...
class ConvNd_MetaBayes(Module):
    ...
    def __init__(self,
                  ...
                  dtype=None) -> None:
        factory_kwargs = {'device': device, 'dtype': dtype}
        super(ConvNd_MetaBayes, self).__init__()
    ...
    def reset_parameters(self) -> None:
        ...
        self.bound = math.sqrt(2/fan_in)
        self.sigma_init = 0.5*math.sqrt(1/fan_out)
        init.uniform_(self.weight_mu, -self.bound, self.bound)
        init.constant_(self.weight_sigma ,self.sigma_init)
    ...
    def __setstate__(self, state):
        super(ConvNd_MetaBayes, self).__setstate__(state)
...
class Conv2d_MetaBayes(ConvNd_MetaBayes):
    ...
    def __init__(
        ...
        super(Conv2d_MetaBayes, self).__init__(
            in_channels, out_channels, kernel_size_, stride=stride_, padding=padding_, dilation=dilation_,
